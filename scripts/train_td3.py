...
def main(args):
    ...
    norm_train = dict(norm_obs=True, norm_reward=False, clip_obs=10.0, training=True)
    norm_eval  = dict(norm_obs=True, norm_reward=False, clip_obs=10.0, training=False)

    # action_noise 的维度取决于控制器，cart=7，joint=7（UR5e 6关节+夹爪）
    n_actions = 7
    ...
    save_run_config(
        dict(exp=args.exp_name, total_timesteps=args.total_timesteps,
             num_cpu=args.num_cpu, norm_train=norm_train,
             env_config=env_config, td3_config=td3_config
        ),
    ...
    model = TD3(env=train_env, tensorboard_log=tb_log_dir, **td3_config)
    try:
...
if __name__ == "__main__":
    ...
    p.add_argument("--exp_name",          type=str,   default="td3_lift")
    p.add_argument("--seed",              type=int,   default=0)
    p.add_argument("--num_cpu",           type=int,   default=6)
    p.add_argument("--total_timesteps",   type=int,   default=2_000_000)
    p.add_argument("--learning_rate",     type=float, default=3e-4)         # 默认是1e-3，但这里3e-4，也能跑通 
    ...
    p.add_argument("--action_noise",      type=float, default=0.2)          # 探索不足可适当增大
    p.add_argument("--device",            type=str,   default="cuda")
    p.add_argument("--eval_steps",        type=int,   default=163840)
    p.add_argument("--model_save_freq",   type=int,   default=500_000)
    p.add_argument("--action_penalty",    type=float, default=0)            # joint控制取消惩罚
